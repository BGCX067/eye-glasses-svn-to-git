...
public class SettingAction extends BaseAdminAction {
	...
	@Validations(
		...
		intRangeFields = {
			...
			@IntRangeFieldValidator(fieldName = "setting.buildHtmlDelayTime", min = "1", message = "HTML自动生成延时必须为正整数!")
		},
		emails = {
	...
	public String update() throws Exception {
		...
		if (defaultBigGoodsImage != null) {
			String defaultBigGoodsImagePath = ImageUtil.copyImageFile(getServletContext(), defaultBigGoodsImage);
			persistent.setDefaultBigGoodsImagePath(defaultBigGoodsImagePath);
		}
		if (defaultSmallGoodsImage != null) {
			String defaultSmallGoodsImagePath = ImageUtil.copyImageFile(getServletContext(), defaultSmallGoodsImage);
			persistent.setDefaultSmallGoodsImagePath(defaultSmallGoodsImagePath);
		}
		if (defaultThumbnailGoodsImage != null) {
			String defaultThumbnailGoodsImagePath = ImageUtil.copyImageFile(getServletContext(), defaultThumbnailGoodsImage);
			persistent.setDefaultThumbnailGoodsImagePath(defaultThumbnailGoodsImagePath);
		}
